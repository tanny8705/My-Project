...
export async function apiAdminStudentsCsv(token, params = {}) {
  const q = new URLSearchParams(params);
  const qs = q.toString() ? `?${q.toString()}` : "";
  return downloadCsv(token, `/api/admin/reports/students.csv${qs}`);
}

export async function apiScopedStudentsCsv(token) {
  return downloadCsv(token, `/api/reports/students.csv`);
}

export async function apiAdminVerificationsCsv(token, params = {}) {
  const q = new URLSearchParams(params);
  const qs = q.toString() ? `?${q.toString()}` : "";
  return downloadCsv(token, `/api/admin/reports/verifications.csv${qs}`);
}

export async function apiAdminActorsCsv(token, role) {
...
export async function apiAdminDepartmentUpdate(token, id, payload) {
  const r = await fetch(`${API_BASE}/api/admin/departments/${id}`, {
    method: "PUT",
    headers: authHeaders(token),
    body: JSON.stringify(payload),
